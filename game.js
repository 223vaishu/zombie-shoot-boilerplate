...
const expAudio = new Audio("shotgun.wav");
expAudio.volume = 0.2;
gameBody.onclick = () => {
  expAudio.pause();
...
function makeZombie() {
    ...
    gameBody.innerHTML += `<img src="${randomImage}" class="zombie-image" id="zombie${zombieId}">`;
    let zombie = document.getElementById("zombie" + zombieId);
    zombie.style.transform = `translateX(${getRandomInt(20, 80)}vw)`;
    zombie.style.animationDuration = `${getRandomInt(7,9)}s`;
    zombie.onclick = () => {
